...
function! BigGrep(corpus, query, search)
  ...
  let &grepprg = a:corpus . "bg" . a:query . " -i '$*' \\| sed -e '".subst."'"
...
command! TW call BigGrep("t", "s", expand("<cword>"))
command! -nargs=1 T call BigGrep("t","s", <q-args>)
command! -nargs=1 TR call BigGrep("t","r",<q-args>)

command! FW call BigGrep("f", "s", expand("<cword>"))
command! -nargs=1 F call BigGrep("f","s",<q-args>)
command! -nargs=1 FR call BigGrep("f","r",<q-args>)
